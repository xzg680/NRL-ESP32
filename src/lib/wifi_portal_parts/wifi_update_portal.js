const translations={
en:{language:'Language',wifiConfig:'WiFi Config',nrlConfig:'NRL Config',audioSettings:'Audio Settings',firmwareUpdate:'Firmware Update',configAp:'Config AP',stationIp:'Station IP',firmwareVersion:'Firmware version',updateHeadline:'Firmware Update',updateIntro:'Upload a firmware file over this WiFi connection.',updateDoneHeadline:'Update Complete',updateDoneIntro:'Firmware was written successfully. Rebooting shortly.',updateFailHeadline:'Update Failed',updateFailIntro:'Firmware upload failed. Check that the file is a valid firmware.bin.',otaNotice:'Select a firmware.bin file from support or from a trusted release. The device will reboot after a successful update.',remoteOtaTitle:'Remote OTA',remoteOtaIntro:'The device periodically reports its board, firmware, NRL callsign and SSID to the HTTPS OTA server. Only firmware built for this board is listed.',otaServerUrl:'OTA server URL (HTTPS)',deviceAccessToken:'Device access token (optional)',saveOtaServer:'Save OTA server',checkReleases:'Check releases',remoteNotConfigured:'Not configured',availableVersions:'Available versions',checkReleasesFirst:'Check releases first',selectVersion:'Select a version',noCompatibleReleases:'No compatible releases were returned.',installSelectedVersion:'Install selected version',remoteWorking:'Working...',remoteLatest:'Latest version: {version}',none:'none',remoteSavedChecking:'Saved. Checking releases...',remoteChecking:'Checking releases...',confirmInstall:'Install {version}? The device will reboot.',remoteDownloading:'Downloading firmware. The device will reboot.',remoteError:'Error: {error}',requestFailed:'Request failed',useHttpsServerUrl:'Use an HTTPS OTA server URL. The address was not saved.',configureOtaFirst:'Configure the OTA server first.',selectReleasedVersion:'Select a released version.',otaTitle:'WiFi Firmware Update',uploadFirmware:'Upload firmware',otaUploading:'Uploading firmware...',otaInstalling:'Installing firmware, please wait...',otaDone:'Update complete. Device is rebooting.',otaRebooting:'Waiting for device to come back online...',otaRebooted:'Device is back online. Update finished.',otaRebootTimeout:'Could not reach the device. Please check it manually.',otaFailed:'Update failed. Please try again.'},
zh:{language:'语言',wifiConfig:'WiFi配置',nrlConfig:'NRL配置',audioSettings:'音频设置',firmwareUpdate:'固件升级',configAp:'配置热点',stationIp:'联网地址',firmwareVersion:'固件版本',updateHeadline:'固件升级',updateIntro:'通过当前 WiFi 配置页面上传固件文件。',updateDoneHeadline:'升级完成',updateDoneIntro:'固件写入成功，设备即将重启。',updateFailHeadline:'升级失败',updateFailIntro:'固件上传失败，请确认文件是有效的 firmware.bin。',otaNotice:'请选择来自官方发布或售后支持的 firmware.bin 文件。升级成功后设备会自动重启。',remoteOtaTitle:'远程 OTA 升级',remoteOtaIntro:'设备会定期向 HTTPS OTA 服务器报告板卡、固件版本、NRL 呼号和 SSID，并且只显示适用于当前板卡的固件。',otaServerUrl:'OTA 服务器地址（HTTPS）',deviceAccessToken:'设备访问令牌（可选）',saveOtaServer:'保存 OTA 服务器',checkReleases:'检查版本',remoteNotConfigured:'尚未配置',availableVersions:'可用版本',checkReleasesFirst:'请先检查版本',selectVersion:'选择版本',noCompatibleReleases:'服务器没有返回适用于当前板卡的版本。',installSelectedVersion:'安装选中版本（可升级或回退）',remoteWorking:'正在处理...',remoteLatest:'最新版本：{version}',none:'无',remoteSavedChecking:'已保存，正在检查版本...',remoteChecking:'正在检查版本...',confirmInstall:'确定安装 {version} 吗？设备将会重启。',remoteDownloading:'正在下载固件，设备将会重启。',remoteError:'错误：{error}',requestFailed:'请求失败',useHttpsServerUrl:'请输入 HTTPS OTA 服务器地址，当前地址没有保存。',configureOtaFirst:'请先配置 OTA 服务器。',selectReleasedVersion:'请选择已发布的固件版本。',otaTitle:'WiFi固件升级',uploadFirmware:'上传固件',otaUploading:'正在上传固件...',otaInstalling:'正在升级，请稍候...',otaDone:'升级完成，设备正在重启。',otaRebooting:'正在等待设备重新启动...',otaRebooted:'设备已重启完成，升级成功。',otaRebootTimeout:'无法连接到设备，请手动检查。',otaFailed:'升级失败，请重试。'}
};
...
let otaCurrentKey=null,otaCurrentPct=null,lastRemoteStatus=null,remoteMessage=null;
function applyLanguage(l){localStorage.setItem('nrl_lang',l);document.documentElement.lang=l==='zh'?'zh-CN':'en';document.querySelectorAll('input[name="lang"]').forEach((r)=>{r.checked=(r.value===l);});document.querySelectorAll('[data-i18n]').forEach((el)=>{const k=el.getAttribute('data-i18n');if(translations[l]&&translations[l][k])el.textContent=translations[l][k];});const heading=document.querySelector('h1');if(heading)document.title=heading.textContent;if(otaCurrentKey)setOta(otaCurrentKey,otaCurrentPct);renderRemote();}
...
if(checkRemote)checkRemote.addEventListener('click',async()=>{try{const serverUrl=remoteUrl.value.trim();const scheme=serverUrl.toLowerCase();if(scheme.startsWith('http://')||scheme.startsWith('https://')){await remotePost('/ota/config',{server_url:serverUrl,device_token:remoteToken.value});}await remotePost('/ota/check',{});setRemoteMessage('remoteChecking');startRemotePolling();}catch(error){setRemoteMessage('remoteError',{error:localizeApiError(error.message)});}});
