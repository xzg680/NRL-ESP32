...
// NRL-ESP32 port (from ESP32APRS_Audio / VP-Digi):
//  - Arduino, AFSK.h (ADC/DAC/I2S plumbing) and esp_dsp dependencies removed.
//  - RX unchanged: feed 9600 Hz PCM16 samples into MODEM_DECODE().
//  - TX reworked: MODEM_BAUDRATE_TIMER_HANDLER() now generates samples at
//    MODEM_TX_SAMPLE_RATE using Q16 fractional phase/baud accumulators (the
//    original used integer steps at a 38400 Hz DAC timer), so the caller pulls
//    PCM sample-by-sample and pushes frames onto the audio router.
//  - PTT/DCD side effects replaced with callbacks (no LED / timer hardware).
#include <string.h>
#include <math.h>
#include <stdlib.h>
#include <stdbool.h>
#include "aprs_modem.h"
#include "aprs_ax25.h"
#include "esp_log.h"

static const char *TAG = "aprs_modem";
#define log_d(...) ESP_LOGD(TAG, __VA_ARGS__)

#define SIN_LEN 512
static uint8_t sinFull[SIN_LEN]; // unsigned 8-bit sine, filled in ModemInit()
static inline uint8_t sinSample(uint16_t i) { return sinFull[i % SIN_LEN]; }
#define DIV_ROUND(dividend, divisor) (((dividend) + (divisor) / 2) / (divisor))

static ModemPttCallback_t pttCallback = 0;
static ModemDcdCallback_t dcdCallback = 0;
static volatile uint8_t modemTransmitting = 0;

void ModemSetPttCallback(ModemPttCallback_t cb) { pttCallback = cb; }
void ModemSetDcdCallback(ModemDcdCallback_t cb) { dcdCallback = cb; }
uint8_t ModemIsTransmitting(void) { return modemTransmitting; }
...
// PLL tick increment values; stored in an int32 phase accumulator, so the
// N9600=1 case intentionally wraps to 0 (MODEM_9600 is unused in this port).
#define PLL1200_STEP ((int32_t)(((uint64_t)1 << 32) / N1200))
#define PLL9600_STEP ((int32_t)((((uint64_t)1 << 32) / N9600) & 0xFFFFFFFFu))
#define PLL300_STEP ((int32_t)(((uint64_t)1 << 32) / N300))
...
// Q16.16 fractional steps, in sine-table entries (tone) / bits (baud) per
// output sample at MODEM_TX_SAMPLE_RATE
static uint32_t markStep;
static uint32_t spaceStep;
static uint32_t baudStep;
 ...
 */
static uint32_t phaseAcc = 0; // Q16.16 sine table index
static uint32_t baudAcc = 0;  // Q16.16 bit-phase; >= 1.0 means fetch next bit
uint8_t MODEM_BAUDRATE_TIMER_HANDLER(void)
{
	...
	else
	{
		...
		phaseAcc %= ((uint32_t)SIN_LEN << 16);
		sinwave = sinSample(phaseAcc >> 16);
	}
	return sinwave;
}

/**
 * @brief Reset TX generator state before a new transmission
 */
static void txGeneratorReset(void)
{
	phaseAcc = 0;
	baudAcc = (1ul << 16); // fetch the first bit on the first sample
	currentSymbol = 0;
}
...
void ModemTxTestStop(void)
{
	...
	if (pttCallback)
		pttCallback(false); // PTT off
}

/**
 * @brief Start TX. The caller (aprs_service TX pump) sees ModemIsTransmitting()
 * go high and starts pulling samples from MODEM_BAUDRATE_TIMER_HANDLER().
 */
...
/**
 * @brief Stop TX and go back to RX. Called from Ax25GetTxBit() when the frame
 * tail has been sent; the TX pump stops pulling samples once it sees the flag.
 */
...
void ModemInit(void)
{
	...
	markStep = (uint32_t)((float)SIN_LEN * markFreq / (float)MODEM_TX_SAMPLE_RATE * 65536.f);
	spaceStep = (uint32_t)((float)SIN_LEN * spaceFreq / (float)MODEM_TX_SAMPLE_RATE * 65536.f);
	baudStep = (uint32_t)(baudRate / (float)MODEM_TX_SAMPLE_RATE * 65536.f);

	log_d("markStep %lu spaceStep %lu baudStep %lu", (unsigned long)markStep, (unsigned long)spaceStep, (unsigned long)baudStep);
