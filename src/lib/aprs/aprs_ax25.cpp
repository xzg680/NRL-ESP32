...
// NRL-ESP32 port (from ESP32APRS_Audio / VP-Digi): Arduino dependencies
// replaced with ESP-IDF equivalents; FX.25 support not compiled in.
#include <stdlib.h>
#include <stdbool.h>
#include <string.h>
#include "aprs_modem.h"
#include "aprs_ax25.h"
#include "aprs_crc_ccit.h"
#include "esp_log.h"
#include "esp_timer.h"
...
#define _BV(n) (1u << (n))
#endif
static inline unsigned long millis(void) { return (unsigned long)(esp_timer_get_time() / 1000ULL); }
static inline long random(long from, long to) { return from + (long)(esp_random() % (uint32_t)(to - from)); }
...
void Ax25BitParse(uint8_t bit, uint8_t modem,uint16_t mV)
{
	...
	//{
		...
		if((rx->rawData & 0x3F) == 0x3E) //dismiss bit 0 added by bit stuffing
			return;
#ifdef ENABLE_FX25
	} // closes if(rx->rx != RX_STAGE_FX25_FRAME)
#endif
...
void Ax25Init(uint8_t fx25Mode)
{
	...
	txInitStage = TX_INIT_OFF; // upstream had '==' here (no-op); fixed to assign
