...
#ifndef APRS_MODEM_H_
#define APRS_MODEM_H_

#include <stdint.h>
#include <stdbool.h>

// NRL port: PCM sample rates of the modem edges.
// RX: all demodulator FIR coefficients are designed for 9600 Hz — feed
// MODEM_DECODE() with 9600 Hz samples (aprs_service resamples the mic tap).
// TX: MODEM_BAUDRATE_TIMER_HANDLER() generates at this rate; it matches the
// speaker sink rate of the audio router so frames pass through unconverted.
#define MODEM_RX_SAMPLE_RATE 9600
#define MODEM_TX_SAMPLE_RATE 8000

typedef void (*ModemPttCallback_t)(bool on);
typedef void (*ModemDcdCallback_t)(bool on);

// Callbacks replace the original direct PTT-pin / DCD-LED hardware access.
void ModemSetPttCallback(ModemPttCallback_t cb);
void ModemSetDcdCallback(ModemDcdCallback_t cb);

// 1 while a transmission is in progress: the TX pump keeps pulling samples
// from MODEM_BAUDRATE_TIMER_HANDLER() until this drops back to 0.
uint8_t ModemIsTransmitting(void);
