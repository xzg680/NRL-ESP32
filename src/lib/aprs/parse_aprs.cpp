...
std::string ParseAPRS::deg2lon(double deg)
{
	char sign;
	if (deg > 0.0F)
	{
	...
	return std::string(dmm);
}

std::string ParseAPRS::deg2lat(double deg)
{
	char sign;
	if (deg > 0.0F)
	{
...
int ParseAPRS::parse_aprs_wx(struct pbuf_t *pb, char const *input, unsigned int const input_len)
{
	...
	// snow was char[3] upstream but snow[3] is written below (1-byte overflow)
