...
extern "C" bool NRL_AEC_EspSrProbe(void)
{
    ...
    if (models == nullptr) {
        ESP_LOGW(TAG, "model partition unavailable; esp-sr probe skipped");
        return false;
    }
    ...
    if (iface == nullptr || iface->create_from_config == nullptr) {
        ESP_LOGE(TAG, "esp-sr AFE interface unavailable");
        afe_config_free(cfg);
        esp_srmodel_deinit(models);
        return false;
    }
    ...
    ESP_LOGI(TAG, "AFE NSNET2 probe: feed=%d fetch=%d feed_ch=%d "
    ...
    iface->destroy(afe);
    esp_srmodel_deinit(models);
    ESP_LOGI(TAG, "AFE destroyed -- esp-sr runtime probe OK");
    return true;
}
