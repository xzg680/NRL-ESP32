...
/* NRL PATCH: bool Kconfig options emit no macro at all when disabled, which
/  only worked upstream because every use sat behind #if FF_FS_EXFAT (0).
/  With exFAT on, map missing -> 0 explicitly. Same for FF_USE_LABEL below. */
...
/* NRL PATCH: this whole component is a copy of ESP-IDF 6.2's fatfs with
/  ONLY this option flipped 0 -> 1, so factory-exFAT TF cards (>32 GB)
/  mount directly. IDF hardcodes exFAT off with no Kconfig; a project-local
/  component override is the supported way to change it. LFN is already on
/  (CONFIG_FATFS_LFN_HEAP). When upgrading IDF, re-copy components/fatfs
/  from the new IDF and re-apply this one change. */
